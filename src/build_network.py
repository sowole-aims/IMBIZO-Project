...
"""This script Builds multiple  Networks for our CNN.
"""
...
class LNLN(object):
	...
	def compile(self):
		...
		self.dense_2 = tf.contrib.keras.layers.Dense(self.num_cells,activation='relu')
                ...
                # Creating nonlinearity scalers
		with tf.variable_scope('scaled_nonlinear'):
			print("building non linear layer")
			weights1 = tf.get_variable('weights',
				initializer = 0.1+tf.truncated_normal([self.num_units],
					stddev = 0.02))
			...
			weights2 = tf.get_variable('weights2',
				initializer = 0.1+tf.truncated_normal([self.num_cells],
					stddev= 0.02))
...
class LnonL(object):
	...
	def __init__(
		...
		with tf.variable_scope('nonlinear'):
			print("building non linear layer")
			weights = tf.get_variable('weights',
				initializer = 0.1+tf.truncated_normal([ncell],
					stddev= 0.02))
			biases = tf.get_variable('biases', initializer = tf.zeros([ncell]))
			#hidden2_drop = tf.nn.dropout(hidden2, keep_prob)
			#linear = tf.nn.relu(tf.add(tf.matmul(hidden2_drop, weights),biases))
...
class RConvNet(object):
	"""
	A Recurent convolutional neural network to predict neuron firing rates due to images 
	"""
	def __init__(
		...
		# full recurrent convolution network output
		self.output = linear
		
		# linear output of dense layer for pretraining
		with tf.variable_scope('linearpretrain'):
			...
			print("linear maping %d elemements to %d elements" % (numelements, ncell))
		# output for pre training the convolution network
...
class ConvNetDrop(object):
	...
	def __init__(
		...
		with tf.variable_scope('linear'):
			...
			print("linear mapping %d elements to %d elements" % (dim, ncell))
		self.output = linear
...
class ConvNet(object):
	...
	def __init__(
		...
		for i in range(len(dense_list)):
			...
			with tf.variable_scope("dense%d" % (i+1)):
				dim = previous_layer.get_shape()[1].value
				print("mapping %d elements to %d elements" % (dim, dense_list[i]))
				W = tf.get_variable('W',
					initializer=tf.truncated_normal([dim, dense_list[i]],
						stddev = 0.1 / math.sqrt(float(dim))))
				b = tf.get_variable('b', initializer = tf.zeros([dense_list[i]]))
...
class DenseLayers(object):
	...
	def __init__(self, input, dense_list):
		...
		# number of input channels
		#dum,dum,dum, imgchannels = tf.shape(images)
		print("start building densely connected layers")
		...
		for i in range(len(dense_list)):
			...
			with tf.variable_scope("dense%d" % (i+1)):
				dim = previous_layer.get_shape()[1].value
				print("mapping %d elemements to %d elements" % (dim, dense_list[i]))
				weights = tf.get_variable('weights',
					initializer=tf.truncated_normal([dim, dense_list[i]],
						stddev=0.1 / math.sqrt(float(dim))))
