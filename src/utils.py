...
class DataLoader(object):
	...
	def __init__(self, filenamelist,FLAGS = None):	
		...
		## create an object with training, early stop and evaluation formatted data 
		...
		activity = mat_contents['activity'] # the raw outputs (firing rates of neurons), update as needed
		# activity data formated as array with: [number of images, number of neurons]  
		images = mat_contents['images'] # the raw inputs (image), update as needed 
		# images data formated as: [number of image, number or x pixels, number of y pixels].
		# for black and white images 
		activitydum = activity
		ncellsdum = np.shape(activitydum)[1]
		print([filename + " ncells:"])
		print(ncellsdum)
		if numfiles > 0: # for concatinating several files. For adding new outputs (neurons) for same inputs (image)
		...
		numearlystop = int(np.floor(numimg*FLAGS.earlystop_frac))
		numeval =  numimg - numtrain - numearlystop
		...
		xstop = np.reshape(x[earlystoptrials,:,:],(numearlystop,numpixx,numpixy,1))
		...
		self.numearlystop = numearlystop
